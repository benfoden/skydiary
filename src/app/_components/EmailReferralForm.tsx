  ...
  return (
    <form
      action={async (formData) => {
        "use server";
        const friendEmail: string = formData.get("friendEmail") as string;
        const friendEmail2: string =
          (formData.get("friendEmail2") as string) ?? "";

        const firstName: string = formData.get("firstName") as string;
        const sendTos = [friendEmail, friendEmail2].filter(
          (email) => email !== "",
        );
            ...
            await sendEmail({
              ...
              htmlBody: `<body style="font-family: sans-serif; background: linear-gradient(to bottom, #cce3f1, #F3F6F6) no-repeat; background-size: cover; color: #000; padding: 32px 16px; text-align: center;">
                          <div style="padding: 32px; max-width:600px;">
                            ...
                            <p style="font-size: 16px; margin-bottom: 16px;">skydiary is a daily journal with strong privacy and personalized AI advisors.</p>
                            <p style="font-size: 16px; ">it's independently run by a husband and wife team based in Kyoto, Japan.</p>
