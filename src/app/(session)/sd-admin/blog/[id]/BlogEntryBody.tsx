...
export default function BlogEntryBody({ post }: { post: BlogPost }) {
  ...
  const { data, isSuccess } = api.blogPost.getByPostId.useQuery({
    postId: post?.id,
  });
  ...
  const handleContentChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const newTimeout = setTimeout(() => {
      updateBlogPost.mutate({ content: newContent, postId: post?.id });
    }, 1000);
